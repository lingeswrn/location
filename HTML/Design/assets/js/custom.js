...
// Render hero search form ---------------------------------------------------------------------------------------------
    ...
    $("select").on("rendered.bs.select", function () {
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', '//cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.12.2/css/bootstrap-select.min.css') );
        if( !viewport.is('xs') ){
    ...
    if( $(".calendar").length ){
        ...
        for( var i = 1 ; i<=12 ; i++ ){
            ...
            $("#month_"+i).zabuto_calendar({
                ajax: {
                    url: "assets/php/calendar.php.html",
                    modal: true
                },
                action: function () {
    ...
    $(".form-email .btn[type='submit']").on("click", function(){
        ...
        form.validate({
            submitHandler: function() {
                $.post("assets/external/email.php.html", form.serialize(),  function(response) {
